...
const parseToSQL = (data) => {
  ...
  for (beer of data) {
    ...
    const beer_query = `INSERT INTO beer VALUES(default, '${beer.name.replaceAll("'", "''")}', '${beer.tagline.replaceAll(
      "'",
      "''"
    ...
    const fermentation_temp_value = method.fermentation.temp.value ? `'${method.fermentation.temp.value}'` : null;
    const fermentation_temp_unit = method.fermentation.temp.unit ? `'${method.fermentation.temp.unit}'` : null;
